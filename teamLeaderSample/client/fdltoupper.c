...
main(int argc, char *argv[])
{
	...
	if(fc < 0)
		printf("fbput failed [%s]\n", fbstrerror(fberror));

	if (tpcall("FDLTOUPPER", (char *)sndbuf, 0, (char **)&rcvbuf, &rcvlen, 0) == -1)
	{ 
		printf("Can't send request to service FDLTOUPPER\n[tperrno(%d) : %s]\n", tperrno,tpstrerror(tperrno));
		tpfree((char *)sndbuf);
		tpfree((char *)rcvbuf);
		tpend();
		exit(1);
